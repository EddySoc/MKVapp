...
def show_color_preview(shared):
    ...
    for name, data in shared.entry_data.items():
        ...
        try:
            ...
            tk.Label(preview, text=f"{name}: ⚠️ {e}").pack()
...
def dump_text(lines, output_widget):
    ...
    if output_widget:
        try:
            cur_state = None
            try:
                cur_state = output_widget.cget("state")
            except Exception:
                cur_state = None
            try:
...
def open_debug_dashboard(app, shared, binding_manager):
    ...
    if str(theme).lower().startswith("dark"):
        out_bg, out_fg = "black", "white"
    else:
        out_bg, out_fg = "#f0f0f0", "#202020"

    output = tk.Text(dash, wrap="word", height=24, bg=out_bg, fg=out_fg)
