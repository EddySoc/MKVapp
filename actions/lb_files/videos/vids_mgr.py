    ...
    try:
        ...
        if os.path.exists(config_path):
            with open(config_path, 'r') as f:
                ...
                # Launch video player with the video file
                try:
                    subprocess.Popen([player_path, video_path], shell=False)
                    print(f"▶️ Playing: {os.path.basename(video_path)}")
                except Exception as e:
                    print(f"❌ Failed to launch video player: {e}")
        else:
...
def transform_2_mkv():
    ...
    try:
        ...
        tb_update('tb_info', "✅ Transform completed - list refreshed", "normal")
...
def mkv_embed_sub():
    ...
    for idx, video_path in enumerate(selected):
        ...
        # Look for matching SRT file (same name, .srt extension)
        dir_name = os.path.dirname(video_path)
        base_name = os.path.splitext(os.path.basename(video_path))[0]
...
def mkv_2_8bitHEVC():
    ...
    tb_update('tb_info', "✅ 8-bit conversion completed", "normal")
...
def mkv_check_lang():
    ...
    if total_subs_found == 0:
        ...
        tb_update('tb_info', "✅ Language check completed", "normal")
