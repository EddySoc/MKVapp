...
class MenuDashboard(ct.CTkToplevel):
    def __init__(self, master=None):
        ...
        self.geometry("1100x640+120+120")
        ...
        # Dropdown of popup keys (widgets)
        ...
        self.items_tb = tk.Text(left, wrap="word", height=20, bg=it_bg, fg=it_fg)
        ...
        self.triggers_list = tk.Listbox(right, height=20)
    ...
    def on_menu_selected(self, key):
        # Show items using the composition so separators and titles are visible
        ...
        if not parts:
            self.items_tb.insert("1.0", "(no items)")
        else:
            for part in parts:
                if part == "__title__":
                    ...
                    for label in labels:
                        # skip placeholder labels or placeholder actions
                        if not label:
                            continue
                        ...
                        if callable(func):
                            ...
                            name = str(func)
                        # also skip entries that point to a placeholder action name
                        if name == "_placeholder_action":
                            continue
                        self.items_tb.insert("end", f"  • {label} → {name}\n")
        self.items_tb.configure(state="disabled")

        # Show triggers: scan app widget tree for custom_name == key
        triggers = self.find_triggers_for_key(key)
        # Populate listbox with triggers (red items)
        self.triggers_list.delete(0, tk.END)
        if not triggers:
            self.triggers_list.insert(tk.END, "(no runtime triggers found)")
        else:
    ...
    def show_composition(self):
        ...
        self.items_tb.delete("1.0", "end")
        if not parts:
            self.items_tb.insert("1.0", "(no composition)")
        else:
            for p in parts:
