...
class BaseTBox(tk.Frame):
    ...
    def on_mouse_click(self, event):
        ...
        # 📂 Get base path (try shared module first, then shared object)
        base_path = getattr(shared, "base_path", None) or getattr(s, "base_path", None)
        if not base_path:
        ...
        if hasattr(shared, 'folder_path_map'):
            # Try exact match first
            if folder_key in shared.folder_path_map:
                full_path = shared.folder_path_map[folder_key]
