    ...
    def _setup_popup_bindings(self):
        ...
        def on_right_click(event):
            ...
            if hasattr(s, 'pop_menu') and s.pop_menu:
                # Always pass self (FilterListBox) which has custom_name = "lb_files"
                # This ensures we use the correct menu composition regardless of which child widget was clicked
                debug_print(f"🖱️ Right-click on lb_files - event.widget: {event.widget.winfo_name()}, using self: {self}, custom_name: {getattr(self, 'custom_name', 'NONE')}", "binding")
