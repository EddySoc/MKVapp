...
def fast_scandir(app,path):
    ...
    def process_files():
        ...
        s.gui_queue.put(lambda: apply_segment_filter())
...
def update_files_from_selected_folder(event):
    ...
    try:
        ...
        if os.path.isdir(selected_path):
            ...
            smart_name = getattr(s, "last_entry_name", None)
            entry_info = s.entry_data.get(smart_name)
            if entry_info and "entry" in entry_info:
                smart = entry_info["entry"]
                print("[DEBUG] voor set_value: entry inhoud =", smart.get())
                smart.set_value(selected_path)
                print("[DEBUG] na set_value: entry inhoud =", smart.get())
                smart.force_redraw()
                print(f"[DEBUG] force_redraw called for {smart_name}")
