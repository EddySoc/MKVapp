...
def debug_print(message, category="general"):
    ...
    try:
        ...
        # Filter to only show lines with ✅ or starting with " - "
        if not ("✅" in message or message.startswith(" - ")):
            return
        ...
        if output_mode in ["textbox", "both"]:
            try:
                ...
                # Fallback: do not print to console, just log the error silently
                pass
...
def set_debug_mode(enabled=True, output="console", categories=None):
    ...
    try:
        ...
        s.config_mgr.save("persistent_cfg")
