...
class Config_Frame(ct.CTkFrame):
    ...
    def _build_basedir_row(self, row):
        ...
        # Bepaal correct Settings pad voor zowel script als exe
        if getattr(sys, 'frozen', False):  # Running as PyInstaller exe
            settings_dir = Path(sys.executable).parent / "Settings"
        else:  # Running as script
            base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            settings_dir = Path(base_dir) / "Settings"
        persistent_path = settings_dir / "persistent_cfg.json"
        ...
        def save_basedir(path):
            # Save to persistent_cfg.json in the correct Settings folder
            try:
                if persistent_path.exists():
                    with open(persistent_path, "r", encoding="utf-8") as f:
                        data = json.load(f)
                ...
                log_settings(f"✅ BaseDir changed to: {path}")
                # [DEBUG] print verwijderd
            except Exception as e:
