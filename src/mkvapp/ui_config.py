...
from customtkinter import get_appearance_mode, CTkFrame, CTkButton
...
def build_bottom_row(app, tab, padding, config):
    ...
    # Replace segmented button with individual buttons for color control
    seg_frame = CTkFrame(row, fg_color="transparent")
    seg_frame.grid(row=0, column=5, columnspan=2, sticky="ew")
    seg_frame.grid_columnconfigure((0,1,2), weight=1)

    def seg_command(value):
        s.segbut_var.set(value)
        apply_segment_filter()

    app.segbut_videos = CTkButton(seg_frame, text="Videos", command=lambda: seg_command("Videos"))
    app.segbut_videos.grid(row=0, column=0, sticky="ew", padx=1)

    app.segbut_subs = CTkButton(seg_frame, text="Subtitles", command=lambda: seg_command("Subtitles"))
    app.segbut_subs.grid(row=0, column=1, sticky="ew", padx=1)

    app.segbut_all = CTkButton(seg_frame, text="All", command=lambda: seg_command("All"))
    app.segbut_all.grid(row=0, column=2, sticky="ew", padx=1)
...
def setup_debug_tab(app, tabview, padding):
    ...
    # Terminal functionaliteit alleen voor tb_debug
    # Command history functionaliteit
    app.tb_debug._cmd_history = []
    app.tb_debug._cmd_history_index = None

    PROMPT = "> "
    import re

    def get_last_command(widget):
        # Vind de laatste promptregel (regex, tolerant voor spaties)
    ...
    def set_command_line(widget, cmd):
        # Vervang huidige prompt-regel door nieuwe cmd
        widget.delete("end-2l linestart", "end-2l lineend")
        widget.insert("end-1l linestart", f"{PROMPT}{cmd}")
        widget.mark_set("insert", "end-1c")

    def insert_prompt(widget):
        # Verwijder alles na de laatste niet-lege regel
        lines = widget.get("1.0", "end-1c").splitlines()
        # Zoek laatste niet-lege regel
        last_content_line = len(lines)
        for i in range(len(lines), 0, -1):
            if lines[i-1].strip() != "":
                last_content_line = i
                break
        widget.delete(f"{last_content_line+1}.0", "end")
        # Voeg prompt toe op de allerlaatste regel
        if widget.index("end-1c") == "1.0":
            widget.insert("end", PROMPT)
            widget.mark_set("insert", f"1.{len(PROMPT)}")
        else:
            widget.insert("end", f"\n{PROMPT}")
            last_line = int(widget.index("end-1c").split(".")[0])
            widget.mark_set("insert", f"{last_line}.{len(PROMPT)}")
        widget.see("insert")

    # Patch tb_debug zodat na elke update automatisch een prompt volgt
    orig_insert_with_tags = app.tb_debug.insert_with_tags
    def patched_insert_with_tags(msg, tag="normal"):
        orig_insert_with_tags(msg, tag)
        insert_prompt(app.tb_debug.textbox)
    app.tb_debug.insert_with_tags = patched_insert_with_tags

    def only_allow_typing_after_prompt(event):
        # Voorkom typen buiten de prompt-regel en forceer cursor na prompt
        widget = event.widget
        ...
        for i, line in enumerate(reversed(lines), 1):
            m = re.match(r"^(\s*>\s)", line)
            if m:
                prompt_line = len(lines) - i + 1
                prompt_col = len(m.group(1))
                break
        if prompt_line is None:
            return None
        prompt_start = f"{prompt_line}.{prompt_col}"
        # Forceer cursor altijd na prompt
        if widget.compare(insert_index, "<", prompt_start):
            widget.mark_set("insert", "end-1c")
            return "break"
        # Voorkom enter/nieuwe regels na promptregel
        if event.keysym == "Return":
            line_idx = int(widget.index("insert").split(".")[0])
            if line_idx > prompt_line:
                widget.mark_set("insert", "end-1c")
                return "break"
        return None

    def on_tb_debug_enter(event):
        widget = event.widget
        # Debug: print dat handler wordt aangeroepen
        print("[DEBUG] on_tb_debug_enter called")
        # Vind promptregel en forceer cursorpositie
        lines = widget.get("1.0", "end-1c").splitlines()
        prompt_line = None
        prompt_col = None
        for i, line in enumerate(reversed(lines), 1):
            ...
            if m:
                ...
                break
        if prompt_line is None:
            print("[DEBUG] No prompt line found, inserting prompt.")
            insert_prompt(widget)
            return "break"
        insert_index = widget.index("insert")
        line_idx = int(insert_index.split(".")[0])
        if line_idx != prompt_line:
            print(f"[DEBUG] Cursor not on prompt line (cursor: {line_idx}, prompt: {prompt_line})")
            widget.mark_set("insert", "end-1c")
            return "break"
        cmd = get_last_command(widget)
        print(f"[DEBUG] Command to execute: '{cmd}'")
        if not cmd:
            print("[DEBUG] No command entered, skipping.")
            return "break"
        # Voeg toe aan history als nieuw
        if not app.tb_debug._cmd_history or (app.tb_debug._cmd_history and cmd != app.tb_debug._cmd_history[-1]):
            app.tb_debug._cmd_history.append(cmd)
        app.tb_debug._cmd_history_index = None
        import subprocess
        try:
        ...
        # Direct output in tb_debug, altijd prompt na output
        app.tb_debug.insert_with_tags(f"\n{output}\n")
        return "break"

    def on_tb_debug_up(event):
        if not app.tb_debug._cmd_history:
            return "break"
        if app.tb_debug._cmd_history_index is None:
            app.tb_debug._cmd_history_index = len(app.tb_debug._cmd_history) - 1
        elif app.tb_debug._cmd_history_index > 0:
            app.tb_debug._cmd_history_index -= 1
        cmd = app.tb_debug._cmd_history[app.tb_debug._cmd_history_index]
        set_command_line(event.widget, cmd)
        event.widget.mark_set("insert", "end-1c")
        return "break"

    def on_tb_debug_down(event):
        if not app.tb_debug._cmd_history or app.tb_debug._cmd_history_index is None:
            return "break"
        if app.tb_debug._cmd_history_index < len(app.tb_debug._cmd_history) - 1:
            app.tb_debug._cmd_history_index += 1
            cmd = app.tb_debug._cmd_history[app.tb_debug._cmd_history_index]
        else:
            app.tb_debug._cmd_history_index = None
    ...
    app.tb_debug.textbox.bind("<Return>", on_tb_debug_enter, add=True)
    app.tb_debug.textbox.bind("<Up>", on_tb_debug_up)
    app.tb_debug.textbox.bind("<Down>", on_tb_debug_down)
    app.tb_debug.textbox.bind("<Key>", only_allow_typing_after_prompt)
    # Start prompt
    insert_prompt(app.tb_debug.textbox)
